...
def test_render_full_row():
    ...
    assert render_row(row) == "| 14 | +2 | 7.5 |  | 1 | 0 | +1 | ✓ 25 | ✓ 12.5 |  | ок |"
...
def test_render_no_meds_no_extra_pipes():
    ...
    s = render_row(row)
    # Exact column count: День | Настр. | Сон | Вес | Тревога | Раздр. | Энергия | Алк/Нарк | Заметки = 9 cells
    assert s.count("|") == 10  # 9 cells = 10 pipes


def test_render_energy_signed():
    assert "| +2 |" in render_row(DayRow(day=1, energy=2))
    assert "| -2 |" in render_row(DayRow(day=1, energy=-2))
    assert "| 0 |" in render_row(DayRow(day=1, energy=0))


def test_render_energy_none_is_empty():
    s = render_row(DayRow(day=1))
    # 7th cell (Энергия) is empty
    assert "|  |  |  |  |  |  |  |  |" in s
